...
Inherits Application
	#tag Event
		Sub EnableMenuItems()
		  ...
		  File_ImportCyberXMLFile.Enabled=True
	...
	#tag MenuHandler
		Function File_ImportCyberXMLFile() As Boolean Handles File_ImportCyberXMLFile.Action
		  //--------------------------------
		  //
		  //
		  //--------------------------------
		  Dim dd as Class_Cyber_XML
		  Dim f as FolderItem
		  Dim yy as Boolean
		  Dim dicomClass as Thread_DICOM_Object
		  Dim gg as RTOG_Plan
		  ...
		  dd=new Class_Cyber_XML
		  dicomClass = new  Thread_DICOM_Object
		  
		  
		  yy=dd.Open_XML(f)
		  
		  dicomClass.RT_Plan.add dd.DICOMPlan
		  
		  
		  if yy Then
		    if UBound(dicomClass.RT_Plan)>-1 Then
		      gg=gRTOG.Add_Plan
		      Plan_Index=UBound(gRTOG.Plan)
		      if gRTOG.Plan(Plan_Index).Import_DICOM_Plan(dd.DICOMPlan,cdbl(NthField(gRTOG.DICOM_ImageOrientationPatient,"\",1)),cdbl(NthField(gRTOG.DICOM_ImageOrientationPatient,"\",5)),  dicomClass ) Then
