#tag Window
Begin Window Window_BEAM_Options_DYNVMLC_LeafOffsets
   Backdrop        =   0
   ...
   Visible         =   True
   Width           =   600
   Begin Listbox Listbox_Openings
      ...
      _ScrollWidth    =   -1
   End
   Begin PushButton PushButto_Close
      AllowAutoDeactivate=   True
      Bold            =   False
      Cancel          =   False
      Caption         =   "Close"
...
#tag WindowCode
	#tag Event
		Sub Open()
		  ...
		  findex=Window_BEAM_Options.beam_num
		  
		  z_MLC=DVMLC.ZMIN+DVMLC.ZThick/2
		  
		  iso=gRTOG.Plan(Plan_Index).Beam(findex).Nominal_Isocenter
		  
		  for i=-20 to 20
		    Listbox_Openings.AddRow str(i)
		    correctedleaf=gBEAM.Beams(findex).egs_Input_CM_MLC_RoundedLeaf(i,z_MLC,DVMLC.Leafradius,iso,0)
...
#tag Events PushButto_Close
	#tag Event
		Sub Action()
		  Window_BEAM_Options_DYNVMLC_LeafOffsets.Close
		End Sub
	#tag EndEvent
#tag EndEvents
#tag ViewBehavior
