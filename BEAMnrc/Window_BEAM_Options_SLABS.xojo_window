#tag Window
Begin Window Window_BEAM_Options_SLABS
   Backdrop        =   0
   ...
   Begin TextField EditField_zmin
      ...
      Width           =   224
   End
   Begin TextField EditField_numslabs
      AllowAutoDeactivate=   True
      AllowFocusRing  =   True
      AllowSpellChecking=   False
      AllowTabs       =   False
...
#tag WindowCode
	#tag Event
		Sub Open()
		  ...
		  S=Window_BEAM_MainInputs.BEAM.CMs(i).SLABS
...
#tag Events EditField_zmin
	...
	#tag EndEvent
#tag EndEvents
#tag Events EditField_numslabs
	#tag Event
		Sub TextChange()
		  if donothing=False Then
		    S.N_SLABS=val(me.Text)
		    ...
		    ReDim S.Zthink(S.N_SLABS-1)
		    ReDim S.ECUT(S.N_SLABS-1)
		    ReDim S.PCUT(S.N_SLABS-1)
		    ReDim S.Dose_Zone(S.N_SLABS-1)
		    ReDim S.IRegion_Bit(S.N_SLABS-1)
		    ReDim S.Esave(S.N_SLABS-1)
		    ReDim S.Medium_In(S.N_SLABS-1)
		  end
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events PushButton_Close
	#tag Event
		Sub Action()
		  Window_BEAM_Options_SLABS.Close
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events PushButton_LAYERS
	#tag Event
		Sub Action()
		  Window_BEAM_Options_SLABS_LAYERS.Show
		End Sub
	#tag EndEvent
#tag EndEvents
#tag ViewBehavior
