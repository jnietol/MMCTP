#tag Window
Begin Window Window_BEAM_Options_DYNVMLC_LeafOpenings
   Backdrop        =   0
   ...
   Begin PushButton PushButto_Close
      ...
      Width           =   80
   End
   Begin PopupMenu PopupMenu_fields
      AllowAutoDeactivate=   True
      Bold            =   False
      DataField       =   ""
      DataSource      =   ""
...
#tag WindowCode
	#tag Event
		Sub Open()
		  ...
		  for i=0 to UBound(DVMLC.Fields)
		    PopupMenu_fields.AddRow "Field "+str(i+1)+", index : "+Format(DVMLC.Fields(i).Index,"-#.#####")
		  next
...
#tag Events PushButto_Close
	#tag Event
		Sub Action()
		  Window_BEAM_Options_DYNVMLC_LeafOpenings.Close
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events PopupMenu_fields
	#tag Event
		Sub Change()
		  Pop_Opening
		End Sub
