...
Begin Window Window_XiO_Import
   ...
   Begin GroupBox GroupBox2
      ...
      Begin Label StaticText14
         ...
         Width           =   63
      End
      Begin TextField EditField_machine
         AllowAutoDeactivate=   True
         AllowFocusRing  =   True
         AllowSpellChecking=   False
         AllowTabs       =   False
      ...
      Begin GroupBox GroupBox1
         ...
         Begin Label StaticText121
            ...
            Width           =   107
         End
         Begin TextField EditField_Angle
            AllowAutoDeactivate=   True
            AllowFocusRing  =   True
            AllowSpellChecking=   False
            AllowTabs       =   False
      ...
      Begin Label StaticText241
         ...
         Width           =   98
      End
      Begin TextField EditField_Depth_Offset
         AllowAutoDeactivate=   True
         AllowFocusRing  =   True
         AllowSpellChecking=   False
         AllowTabs       =   False
   ...
   Begin Label StaticText4
      ...
      Width           =   100
   End
   Begin PushButton PushButton_Load_Folder
      AllowAutoDeactivate=   True
      Bold            =   False
      Cancel          =   False
      Caption         =   "Load folder"
...
#tag WindowCode
	#tag Event
		Sub Open()
		  Lastfolder=new FolderItem
		  Lastfolder=SpecialFolder.Desktop
		End Sub
	#tag EndEvent


	#tag Method, Flags = &h0
		Sub Data_Import(k as Integer)
		  //------------------------------
		  // Import XiO profile into McGill RT profile
		  //------------------------------
		  Dim i, num_meas  as Integer
		  Dim pp as Class_Profile_One
		  dim point as Class_Points
		  dim x,y,z_offset,z as Single
		  //------------------------------
		  ...
		  // Offset for phantom edge in XiO
		  z_offset=val(EditField_Depth_Offset.Text)
		  ...
		  pp.Z_label="Position"
		  pp.Y_label="Position"
		  pp.Date=data.exportingdate
		  pp.Linac=EditField_machine.Text
		  pp.Units_x="cm"
		  pp.Units_z="cm"
		  pp.Units_y="cm"
		  pp.Source_TYPE=0
		  pp.Field_X=val(EditField_FSx.Text)
		  pp.Field_Y=val(EditField_FSy.Text)
		  pp.Energy=val(EditField_Energy.Text)
		  pp.Algorithm=EditField_algorithm.Text
		  pp.AddOn=EditField_addon.Text
		  pp.Comment=EditField_Comment.Text
		  pp.SSD=Val(EditField_SSD.Text)
		  
		  if k<0 or k>UBound(data.Graphs) Then
		    Return
		  end
		  if data.Graphs(k).Type=1 and data.Graphs(k).Angle=0 Then
		    // X profile
		    pp.TYPE=2
		    pp.Depth=z_offset-Data.Graphs(k).DPTH
		  elseif data.Graphs(k).Type=2 and data.Graphs(k).Angle=0 Then
		    // Y profile
		    pp.TYPE=3
		    pp.Depth=z_offset-Data.Graphs(k).DPTH
		  elseif data.Graphs(k).Type=1 and abs(data.Graphs(k).Angle)=90 Then
		    // PDD on axial slice
		    pp.TYPE=1
		  elseif data.Graphs(k).Type=2 and abs(data.Graphs(k).Angle)=90 Then
		    // PDD on sagital slice
		    pp.TYPE=1
		  else 
		    pp.TYPE=0
		  end
		  ...
		  for i=0 to UBound(data.Graphs(k).Points)
		    ...
		    Point.X_cm=data.Graphs(k).Points(i).x_cm
		    Point.Y_cm=data.Graphs(k).Points(i).y_cm
		    Point.Z_cm=data.Graphs(k).Points(i).z_cm
		    if pp.TYPE=2 or pp.TYPE=3 Then
		      Point.Z_cm=z_offset-data.Graphs(k).Points(i).z_cm
		    elseif pp.TYPE=1 Then
		      if data.Graphs(k).Angle=-90 Then
		        Point.Z_cm=z_offset+data.Graphs(k).Points(i).z_cm
		      elseif data.Graphs(k).Angle=90 Then
		        Point.Z_cm=z_offset-data.Graphs(k).Points(i).z_cm
		      end
		    end
		  ...
		  if UBound(pp.Points)>0 Then
		    ...
		    x=pp.Pointb.X_cm-pp.Pointa.X_cm
		    y=pp.Pointb.Y_cm-pp.Pointa.Y_cm
		    z=pp.Pointb.Z_cm-pp.Pointa.Z_cm
	...
	#tag Method, Flags = &h0
		Sub Load_all()
		  ...
		  Data=new Class_XiOData
		  
		  if f.Directory Then
		    for i=1 to f.Count
		      LastFolder=f.Item(i)
		      Read_File_XiO(LastFolder)
		    next
		  end
	...
	#tag Method, Flags = &h0
		Sub Read_File_XiO(f as FolderItem)
		  //----------------------------------------------
		  // Readin an XiO file into XiOData file
		  //
		  //----------------------------------------------
		  Dim pp as Class_Profile_One
		  ...
		  graph = new Class_XiO_Graph
		  While ts.EOF=False
		    temp=ts.ReadLine
		    if InStr(Temp, "T:")>0 Then
		      graph.T=val(NthField(Temp,":",2))
		      graph.Type=1
		    elseif InStr(Temp, "S:")>0 Then
		      graph.S=val(NthField(Temp,":",2))
		      graph.Type=2
		    elseif InStr(Temp, "C:")>0 Then
		      graph.C=val(NthField(Temp,":",2))
		      graph.Type=3
		    elseif InStr(Temp, "X:")>0 Then
		      graph.X=val(NthField(Temp,":",2))
		    elseif InStr(Temp, "Y:")>0 Then
		      graph.Y=val(NthField(Temp,":",2))
		    elseif InStr(Temp, "Z:")>0 Then
		      graph.Z=val(NthField(Temp,":",2))
		    elseif InStr(Temp, "Angle:")>0 Then
		      graph.Angle=val(NthField(Temp,":",2))
		    elseif InStr(Temp, "Distance")>0 Then
		      While ts.EOF=False
		        if len(temp)>1 Then
		          ...
		          temp=Replace(temp," ",";")
		          Point= new Class_Points
		          if graph.Type=1 and graph.Angle=0 Then
		            point.X_cm=val(NthField(temp,";",1))
		            point.y_cm=graph.T
		            point.z_cm=graph.Z
		            graph.DPTH=graph.Z
		          elseif graph.Type=1 and abs(graph.Angle)=90 Then
		            // PDD on Axial slice
		            point.z_cm=val(NthField(temp,";",1))
		            point.y_cm=graph.T
		            point.x_cm=graph.X
		            
		          elseif graph.Type=2  and abs(graph.Angle)=0 Then
		            // Profile on Sagial Slice
		            point.y_cm=val(NthField(temp,";",1))
		            point.x_cm=graph.s
		            point.z_cm=graph.Z
		            graph.DPTH=graph.Z
		            
		            
		          elseif graph.Type=2 and abs(graph.Angle)=90 Then
		            // PDD  Sagial Slice
		            point.z_cm=val(NthField(temp,";",1))
		            point.x_cm=graph.s
		            point.y_cm=graph.Y
		          ...
		          elseif graph.Type=3  and abs(graph.Angle)=90 Then
		            point.X_cm=graph.X
		            point.y_cm=graph.Y
		            point.z_cm=val(NthField(temp,";",1))
		          end
		  ...
		  ts.Close
		  data.exportingdate=f.ModificationDate.SQLDate
...
#tag Events EditField_RFA_File
	#tag Event
		Function MouseDown(X As Integer, Y As Integer) As Boolean
		  ...
		  opfi.InitialDirectory=Lastfolder
		  opfi.Title="Select any ASCII file"
		  f2=opfi.ShowModal
	...
	#tag EndEvent
#tag EndEvents
#tag Events PushButton_Load_Folder
	#tag Event
		Sub Action()
		  Load_all
		End Sub
